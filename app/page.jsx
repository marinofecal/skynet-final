    ...
    <div style={{ minHeight: '100vh', background: '#000' }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '80px 24px 60px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '60px', flexWrap: 'wrap', gap: '12px' }}>
          <div className="section-label">SYS:INIT → <span style={{ color: '#39ff14' }}>READY</span></div>
          <div className="section-label" style={{ color: '#4a5568' }}>BRUSSELS · GRC / IFRS / FP&A</div>
        </div>

        <div style={{ marginBottom: '40px' }}>
          <div className="section-label" style={{ marginBottom: '20px' }}>⚡ AI-POWERED FINANCIAL INTELLIGENCE</div>
          <h1 className="display-heading" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(4rem, 11vw, 9rem)', lineHeight: 0.9, letterSpacing: '-0.02em', marginBottom: '20px' }}>
            <span style={{ color: '#00e5ff', display: 'block' }}>SKYNET</span>
            <span style={{ display: 'block', fontSize: '28%', fontWeight: 500, letterSpacing: '0.3em', color: 'rgba(0,229,255,0.45)', marginTop: '12px' }}>
              AUDIT · IFRS · EXCEL
            </span>
          </h1>
        ...
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '56px' }}>
          ...
          <a href="https://linkedin.com/in/crtizgar" target="_blank" rel="noreferrer" className="btn-execute">VIEW PROFILE</a>
        </div>

        {/* SOLO 2 STATS */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 200px))', border: '1px solid rgba(0,229,255,0.1)', background: 'rgba(0,229,255,0.02)', overflow: 'hidden', width: 'fit-content' }}>
          {[
            { value: '3', label: 'AI AGENTS ACTIVE' },
            { value: '15+', label: 'YRS FINANCE EXP' },
          ].map(({ value, label }, i) => (
            <div key={label} style={{ padding: '28px 32px', textAlign: 'center', borderRight: i === 0 ? '1px solid rgba(0,229,255,0.08)' : 'none' }}>
              <div className="display-heading" style={{ fontSize: '2.8rem', color: '#00e5ff', fontFamily: 'var(--font-display)', lineHeight: 1 }}>{value}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
          {BOTS.map((bot) => (
            <Link key={bot.id} href={`/tools/${bot.id}`} style={{ textDecoration: 'none' }}>
              <div
                style={{ background: '#05050f', border: `1px solid rgba(${bot.rgb},0.18)`, padding: '32px', cursor: 'pointer', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', transition: 'all 0.3s ease' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = bot.color; e.currentTarget.style.boxShadow = `0 0 32px ${bot.color}12`; e.currentTarget.style.transform = 'translateY(-4px)'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = `rgba(${bot.rgb},0.18)`; e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
                ...
                <div className="display-heading" style={{ fontSize: '1.6rem', color: 'white', fontFamily: 'var(--font-display)', marginBottom: '4px' }}>{bot.name}</div>
                <div className="section-label" style={{ color: bot.color, marginBottom: '14px' }}>{bot.sub}</div>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.74rem', color: '#4a5568', lineHeight: 1.9, flex: 1, marginBottom: '20px' }}>{bot.desc}</p>

                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
                  {bot.tags.map(tag => (
                    <span key={tag} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.54rem', fontWeight: 700, letterSpacing: '0.1em', padding: '4px 8px', border: `1px solid rgba(${bot.rgb},0.25)`, color: bot.color, background: `rgba(${bot.rgb},0.06)`, whiteSpace: 'nowrap' }}>
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      {/* ABOUT */}
      <section style={{ borderTop: '1px solid rgba(0,229,255,0.08)', background: '#02020a', padding: '64px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 280px', gap: '48px', alignItems: 'start' }}>
          <div>
            ...
            <h2 className="display-heading" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontFamily: 'var(--font-display)', color: 'white', marginBottom: '16px', lineHeight: 1.1 }}>
              Finance Professional.<br /><span style={{ color: '#00e5ff' }}>AI Builder.</span>
            </h2>
