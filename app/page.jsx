...
export default function Home() {
            const bots = [
                      { id: 'audit', name: 'AUDIT AI', icon: '🔍', tagline: 'Risk Intelligence', desc: 'Análisis de riesgos con IA' },
                      { id: 'ifrs', name: 'IFRS AI', icon: '📊', tagline: 'Compliance Engine', desc: 'Normalización contable' },
                      { id: 'excel', name: 'EXCEL AI', icon: '⚙️', tagline: 'Data Forge', desc: 'Transformación de datos' },
                        ];
  ...
  return (
                <main className="w-full bg-black text-white overflow-hidden">
                      ...
                      <section className="relative z-10 min-h-screen flex items-center justify-center px-4">
                              <div className="w-full max-w-4xl text-center">
                                        ...
                                        <div className="flex gap-4 justify-center flex-wrap">
                                                    <button className="px-8 py-3 bg-gradient-to-r from-cyan-500 to-blue-500 text-black font-bold rounded-lg hover:scale-105">Comenzar</button>button>
                                                    <button className="px-8 py-3 border-2 border-cyan-400 text-cyan-400 font-bold rounded-lg hover:bg-cyan-400/10">Explorar</button>button>
                                        </div>div>
                              </div>div>
                      </section>section>
                      ...
                      <section className="relative z-10 py-24 px-4">
                              <div className="max-w-6xl mx-auto">
                                        ...
                                        <Link key={bot.id} href={`/tools/${bot.id}`}>
                                                        <div className="group p-8 rounded-2xl bg-gray-900/30 border border-cyan-500/20 hover:border-cyan-400/60 cursor-pointer transition-all">
                                                                          ...
                                                                          <div className="text-cyan-400 font-semibold group-hover:translate-x-2 transition">Acceder →</div>div>
                                                        </div>div>
                                        </Link>Link>
                                      ))}
                                        </div>div>
                          ...
                          {/* FOOTER */}
                      <footer className="relative z-10 border-t border-cyan-500/20 py-8 text-center text-gray-500 text-sm">
                              <p>© 2026 SKYNET Intelligence Framework</p>p>
                      </footer>footer>
                </main>main>
              );
}</main>
